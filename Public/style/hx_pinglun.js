define(function(require, exports){
	...
	* 绑定 ctrl+enter 发送评论
	*/
	jQ('.textarea-txt').ctrlSubmit(function(event){
		jQ('.subscribe-btn').trigger("click");
	});	
    ...
    jQ('.report-btn').live('click',function () {
		if (isLogin()) {
			var t = jQ(this)
				,postUrl = '/setuser/report?is_ajax=1&random='
				,box = jQ(this).parents('.pinglun-box')
...
//postData = {'type':type,'reportid':rid,'aid':aid,'content':encodeURI(content.substring(0,118))};
	...
	**/
     ...
     *    toauthor-btn 添加删除优问
     **/
    jQ('.view-delete-btn,.view-del-btn,.view-recommend-btn,.toauthor-btn').live('click',function () {
	...
	**/
	jQ('.disagree').each(function(i){
		var box = jQ(this).parents('.pinglun-box')
			...
			;
		if((parseInt(d_num)-parseInt(agree_num)-parseInt(reply_num)) > 5){
