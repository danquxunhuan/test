...
define(function(require, exports){
	...
	*	加载个人标签
	**/
	if(jQ('.territory-tags-wrap').length>0){
		require('hx_per_tags');
	}
	...
	*	5.7
	**/
	if(jQ('.personal-homepage,.individual-center-box,.author-page,.tags-all-list,#tags_list,.tags-nr-box').length>0){
		jQ(document).on('click','.box-delete a,.personal-dingyue .close-ico,.personal-homepage .close-ico,.author-page .close-ico,.set-tags,.add-tag,.tags-dingyue',function(){
	...
	**/
   	if(jQ('#code-img').length>0){
		...
		,url = '/tools/get_capimg.html?&back='+random
		,box = jQ('#code-img');
   		box.click(function(){
	...
	if(jQ('.author-card').length>0){
		jQ('.s-edit').live('click',function(){
			...
			if(jQ(this).find('.s-edit-btn').text()=='修改'){
				if(boxName!='sex'){
					box.html('<input type="text" name="'+boxName+'" class="span2" value="'+boxV+'">')
				}else{
				...
				if(jQ(this).data('oldData') == boxContrast){
					...
					var random = parseInt(Math.random()*100000)
						,url = '/member/edit_profile.html?is_ajax=1&callback='+random
						,postData = {'name':boxName,'val':iptValue}
						;
	...
	**/
    ...
    jQ('.user-manage-btn .btn,.stop-user-btn').live('click',function () {
		if (isLogin()) {
			var t = jQ(this)
				,postUrl = '/setuser/stop_user?is_ajax=1&random='
...
function logout(){
	...
	jQ('.logout-btn').live('click',function(){
		...
		jQ.post(uUrl,function(data){
			var data = eval('(' + data + ')');
			if(data.is_success=='1'){
				jQ('.login-box').before('<div class="login-box"><div class="btn-group"><a href="/contribute" class="contribute" title="投稿">投稿</a></div><div class="btn-group"><a href="#lgnModal" data-toggle="modal">登录</a><a href="http://user.huxiu.com/index.php?m=register">注册</a></div><div class="btn-group"><a href="/rss" title="RSS"><i class="icon-rss"></i></a></div></div>').remove();
