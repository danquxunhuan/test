...
class  Page
{
	...
	protected  $ajax_func_name;   //这个难道是ajax回调函数？
...
protected function _get_link($page,$text)
{
	switch ($this->method) {
		case 'ajax':
			...
			//}
			return '<a onclick="' . $this->ajax_func_name . '(\''.$parameter. $page .'\')" href="javascript:void(0)">' . $text . '</a>' . "\n";
		break;
		...
		case 'ajax备份':
			...
			return '<a onclick="' . $this->ajax_func_name . '(\'' . $page . '\''.$parameter. $page .')" href="javascript:void(0)">' . $text . '</a>' . "\n";
		break;
	...
	*/
    public function show_2(){
        if($this->total_pages != 1){
    		$return = '';
    		$return .= $this->up_page('<');  //首
			for($i = 1;$i<=$this->total_pages;$i++){ //页码循环
				if($i == $this->now_page){ //当前页
					$return .= "<a class='zy_amo'>$i</a>\n";
				}else{ //其他页
