...
<style type="text/css"><!--
#thumpic{ border:1px solid #ddd; width:210px; height:143px;}
        ...
        <li>&nbsp;&nbsp;&nbsp;<a href="__APP__/Institution" >教育机构</a></li>
                ...
                <div id="Right"  class="right">
				   <div class="contentArea">
                    <div class="tab1">
                        <div class="current">编辑文章</div>
                    </div>
<form action="__URL__/edit" method="post" enctype="multipart/form-data">
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
        <tr height="34">
            <th width="100">文章标题：</th>
            <td><input id="title" type="text"  class="input" size="60" name="title" value="<?php echo ($info["title"]); ?>"/> <a href="javascript:void(0)" id="checkArticleTitle">检测是否重复</a></td>
        </tr>
        <tr height="34">
            <th>指定作者ID：</th>
            <td>
            <input id="uid" type="text" name="uid" class="input" value="<?php echo ($info["uid"]); ?>" /><a href="javascript:void(0)" id="checkUid"> 查看作者姓名</a>
            </td>
        </tr>
        
        <?php if(!empty($info['contact'])): ?><tr height="34"><th>投稿人联系方式：</th><td><span style="color:red">（<?php echo ($info["contact"]); ?>）</span></td></tr><?php endif; ?>
        
        <tr height="34">
            <th width="100">文章发布状态：</th>
            <td><label><input  type="radio" name="status" value="0" <?php if($info["status"] == 0): ?>checked="checked"<?php endif; ?> /> 未审核</label> &nbsp; <label><input type="radio" name="status" value="1" <?php if($info["status"] == 1): ?>checked="checked"<?php endif; ?> /> 已发布</label> </td>
        </tr>
            <tr height="34">
            <th width="100">文章显示级别：</th>
            <td><label><input type="checkbox" name="flag[]" value="a" <?php if(strstr($info['flag'],'a')){echo "checked='checked'";} ?> /> 推荐</label> &nbsp;
               <label><input type="checkbox" name="flag[]" value="t" <?php if(strstr($info['flag'],'t')){echo "checked='checked'";} ?> /> 头条</label>
               <label><input type="checkbox" name="flag[]" value="j" <?php if(strstr($info['flag'],'j')){echo "checked='checked'";} ?> /> 精品</label>
               <label><input type="checkbox" name="flag[]" value="h" <?php if(strstr($info['flag'],'h')){echo "checked='checked'";} ?> /> 热门</label></td>
        </tr>
            <tr height="34">
            <th>文章所属分类：</th>
            <td>
                <select name="cid">
                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; if($vo[cid] == $info[cid]): ?><option value="<?php echo ($vo["cid"]); ?>" selected="selected"><?php echo ($vo["name"]); ?></option>
                        <?php else: ?>
            ...
            <tr height="34">
            <th width="100">点击量：</th>
            <td><label><input type="text" class="input" name="click" value="<?php echo ($info["click"]); ?>" /> 次</label> &nbsp;
                 </td>
        </tr>
        <tr height="34">
            <th>文章标签：</th>
            <td><input type="text" class="input" size="60" name="tag" value="<?php echo ($info["tag"]); ?>"/></td>
        </tr>
        <tr height="34">
            <th>文章关键词：</th>
            <td><input type="text" class="input" size="60" name="keywords" value="<?php echo ($info["keywords"]); ?>"/> 多关键词间用半角逗号（,）分开，可用于做文章关联阅读条件</td>
        </tr>
        <tr>
            <th>文章描述：</th>
            <td><textarea class="input" style="height: 60px; width:500px;" name="description"><?php echo ($info["description"]); ?></textarea> 用于SEO的description</td>
        </tr>
        <tr>
            <th>文章摘要：</th>
            <td><textarea class="input" style="height: 60px; width:500px;" name="summary"><?php echo ($info["summary"]); ?></textarea> 如果不填写则系统自动截取文章前200个字符</td>
        </tr>
		...
		<tr height="36">
              <th>新闻图片：</th>
               <td>
                ...
                <input type="text" value="<?php echo ($info["firstimage"]); ?>" class="input"  />
                <img id="thumpic" src="__ROOT__/Uploads/article/<?php echo ($info["firstimage"]); ?>" alt="新闻图片" /><span id="pic" class="prompt"></span>    
              </td>
        </tr>
        ...
        <tr>
            <th>文章内容：</th>
            <td><textarea id="content" class="input" style="height: 300px; width:80%;" name="content"><?php echo ($info["content"]); ?></textarea></td>
        </tr>
